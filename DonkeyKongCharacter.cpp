...
void ADonkeyKongCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAxis("TurnRate", this, &ADonkeyKongCharacter::TurnAtRate);
	PlayerInputComponent->BindAxis("LookUp", this, &APawn::AddControllerPitchInput);
	PlayerInputComponent->BindAxis("LookUpRate", this, &ADonkeyKongCharacter::LookUpAtRate);
...
void ADonkeyKongCharacter::FireProyectil()
{
	...
	posicion.Y += 25;
	posicion.X += 50;
	AProyectil* SpawnedProyectil = GetWorld()->SpawnActor<AProyectil>(AProyectil::StaticClass(), posicion, GetActorRotation());
...
void ADonkeyKongCharacter::OnResetVR()
{
	// If DonkeyKong is added to a project via 'Add Feature' in the Unreal Editor the dependency on HeadMountedDisplay in DonkeyKong.Build.cs is not automatically propagated
...
void ADonkeyKongCharacter::TouchStarted(ETouchIndex::Type FingerIndex, FVector Location)
{
		Jump();
}

void ADonkeyKongCharacter::TouchStopped(ETouchIndex::Type FingerIndex, FVector Location)
{
		StopJumping();
}

void ADonkeyKongCharacter::TurnAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
	AddControllerYawInput(Rate * BaseTurnRate * GetWorld()->GetDeltaSeconds());
}

void ADonkeyKongCharacter::LookUpAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
	AddControllerPitchInput(Rate * BaseLookUpRate * GetWorld()->GetDeltaSeconds());
}

void ADonkeyKongCharacter::MoveForward(float Value)
{
